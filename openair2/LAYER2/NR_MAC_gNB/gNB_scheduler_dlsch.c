...
void nr_store_dlsch_buffer(module_id_t module_id,
  ...
  UE_iterator(RC.nrmac[module_id]->UE_info.list, UE) {
    ...
    for (int i = 0; i < sched_ctrl->dl_lc_num; ++i) {
      ...
      LOG_I(MAC,
            "[gNB %d][%4d.%2d][RLC_BUFFER] %s%d->DLSCH, RLC status for UE %6d: %8d bytes in buffer, total DL buffer size = %d bytes, %d total PDU bytes, %s TA command\n",
            module_id,
            frame,
            slot,
            lcid < 4 ? "DCCH":"DTCH",
...
void nr_schedule_ue_spec(module_id_t module_id,
  ...
  UE_iterator(UE_info->list, UE) {
    ...
    LOG_I(NR_MAC,
          "%4d.%2d [DLSCH/PDSCH/PUCCH] RNTI %04x DCI L %d start %3d RBs %3d startSymbol %2d nb_symbol %2d dmrspos %03x MCS %2d nrOfLayers %d TBS %4d HARQ PID %2d round %d RV %d NDI %d dl_data_to_ULACK %d (%d.%d) PUCCH allocation %d TPC %d\n",
          frame,
          slot,
          rnti,
          sched_ctrl->aggregation_level,
